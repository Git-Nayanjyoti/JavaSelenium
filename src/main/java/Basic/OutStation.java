...
public class OutStation {
	...
	public static void main(String[] args) throws InterruptedException, ParseException, AWTException {
		...
		driver.findElement(By.id("local_dest_name"))
				.sendKeys("Wankhede Stadium, Vinoo Mankad Road, Churchgate, Mumbai, Maharashtra, India");
		Thread.sleep(3000); 
		...
		for (int i = 0; i < option2.size(); i++) {
			...
			if (spans.get(1).getText()
					.equals("Wankhede Stadium, Vinoo Mankad Road, Churchgate, Mumbai, Maharashtra, India")) {
				spans.get(1).click();
			}
		}
		...
		Thread.sleep(2000);
		dateAndTime("21-October-2021", "11:30");
		
		Thread.sleep(2000);
		driver.findElement(By.id("OSLeadGen_DoJEnd")).click();
		
		Thread.sleep(2000);
		dateAndTime("22-October-2021", "2:30");
		
		Thread.sleep(2000);
		driver.findElement(By.id("numberOfPax")).sendKeys("40");
